...
1. `../wiki/SKILL.md`와 `workflows.md`, `metadata.md`, `flashcard.md`, `obsidian-cli.md` reference를 읽는다.
2. 경로 가드 후 기본적으로 read-only 검사를 수행한다.
3. orphans, unresolved links, tags, frontmatter, summary, stale active 페이지, 빈약한 페이지, 중복, flashcard 문법, index 정합성을 검사한다.
